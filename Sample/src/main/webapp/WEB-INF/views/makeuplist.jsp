...
<body>
	<div id="wapper">
		...
		<nav>
			<div class="menu-items">
    			...
    			<h5><a href="makeuplist" class="red-text" onclick="golist()" >Make Up</a></h5>
    ...
    <c:forEach var="makeup" items="${makeup}" varStatus="status">
        <tr>
            ...
            <td>
            <fmt:parseDate value="${makeup.regdate}" var="regdate" pattern="yyyy-mm-dd"/>
            <fmt:formatDate value="${regdate}" pattern="yyyy.mm.dd"/>
            </td>
            
            <td>
            <a href="#" class="detail-link" data-seq-id="${makeup.seq_id}">
                ${makeup.banner_title}
            </a>
            </td>
            <td>${makeup.company_yn}</td>
    ...
    $(document).ready(function() {
        ...
        $('.detail-link').click(function() {
            ...
            window.location.href = '/makeupdetail?seq_id=' + seqId;
        });
    });
...
//     });
    ...
    $("#insertButton").click(function() {
    	window.location.href = '/makeupinsert'; // fashioninsert.jsp로 이동
    });
    
    function goSearch(){
    	let search = $("#searchInput").val();
    	console.log("search" + search);
    	$(location).attr('href',"<c:url value='/makeuplist?search="+search+"'/>");
    }
    function goPage(pageNo){
    	let searchInputValue = $("#searchInput").val();
    	$(location).attr('href',"<c:url value='/makeuplist?pageNo="+pageNo+"'/>");
    }
    function golist() {
